...
/// struct for typed errors of method [`get_event_info`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetEventInfoError {
    Status400(),
    Status401(),
...
pub async fn get_event_info(configuration: &configuration::Configuration, event_key: &str) -> Result<models::EventInfo, Error<GetEventInfoError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_event_key = event_key;

    let uri_str = format!("{}/event/{eventKey}/info", configuration.base_path, eventKey=crate::apis::urlencode(p_path_event_key));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::EventInfo`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::EventInfo`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetEventInfoError> = serde_json::from_str(&content).ok();
...
pub async fn update_event_info(configuration: &configuration::Configuration, event_key: &str, event_info: models::EventInfo) -> Result<(), Error<UpdateEventInfoError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateEventInfoError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
