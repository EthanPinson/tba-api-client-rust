...
/// struct for typed errors of method [`update_team_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateTeamListError {
    Status400(),
    Status401(),
...
pub async fn update_team_list(configuration: &configuration::Configuration, event_key: &str, request_body: Vec<String>) -> Result<(), Error<UpdateTeamListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_event_key = event_key;
    let p_body_request_body = request_body;

    let uri_str = format!("{}/event/{eventKey}/team_list/update", configuration.base_path, eventKey=crate::apis::urlencode(p_path_event_key));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<UpdateTeamListError> = serde_json::from_str(&content).ok();
