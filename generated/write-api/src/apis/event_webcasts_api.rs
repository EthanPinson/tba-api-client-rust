...
/// struct for typed errors of method [`add_event_webcasts`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum AddEventWebcastsError {
    Status400(),
    Status401(),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`delete_event_webcasts`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DeleteEventWebcastsError {
    Status400(),
    Status401(),
    UnknownValue(serde_json::Value),
}
...
pub async fn add_event_webcasts(configuration: &configuration::Configuration, event_key: &str, webcast_update: models::WebcastUpdate) -> Result<(), Error<AddEventWebcastsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_event_key = event_key;
    let p_body_webcast_update = webcast_update;

    let uri_str = format!("{}/event/{eventKey}/webcasts/update", configuration.base_path, eventKey=crate::apis::urlencode(p_path_event_key));
    ...
    req_builder = req_builder.json(&p_body_webcast_update);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<AddEventWebcastsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Remove specified webcasts from an event's webcast list
pub async fn delete_event_webcasts(configuration: &configuration::Configuration, event_key: &str, webcast_update: models::WebcastUpdate) -> Result<(), Error<DeleteEventWebcastsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_event_key = event_key;
    let p_body_webcast_update = webcast_update;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DeleteEventWebcastsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
